  ...
  async function fetchNewCatgirl() {
    ...
    try {
      const response = await fetch(
        "https://nekos.moe/api/v1/random/image?count=1&nsfw=false"
      );
      ...
      if (data.images && data.images.length > 0) {
        ...
        catgirlContainer.innerHTML =
          '<p class="loading-text">No catgirls found :(</p>';
      }
    } catch (error) {
      console.error("Failed to fetch catgirls:", error);
      catgirlContainer.innerHTML =
        '<p class="loading-text">Failed to load catgirls. API might be down.</p>';
    }
  }
